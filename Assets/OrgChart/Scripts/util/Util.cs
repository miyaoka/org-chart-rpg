...
public static class Util {
  ...
  //h/s/l: 0 to 1
  public static Color correctHSVToRGB(float hAngle, float saturation, float luminance){
    var uv = rectPolar (hAngle, saturation);
    return(YUVToRGB (luminance, uv.x * .5f, uv.y * .5f));
  }

  //矩形型の極座標
  //angle 0-1, len 0-1
  public static Vector2 rectPolar(float angle, float length){
    var rad = angle * Mathf.PI * 2f;
    var sin = Mathf.Sin(rad);
    var cos = Mathf.Cos(rad);

    var angle2 =  (angle + 1f/8f) % 1f;
    var edge = angle2 < 1f/4f ? cos
      : angle2 < 2f/4f ? sin
      : angle2 < 3f/4f ? -cos
      : -sin;

    return new Vector2(cos * length / edge, sin * length / edge);
  }
  //y: 0 to 1, u/v: -.5 to .5
  public static Color YUVToRGB(float y, float cb, float cr){
    return clampColor( new Color(
      y + 1.13983f * cr,
      y - 0.39465f * cb - 0.58060f * cr,
      y + 2.03211f * cb
    ));
  }

  private static Color clampColor(Color col){
